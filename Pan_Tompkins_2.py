...
filename = patient_number
record = wfdb.rdrecord(filename, sampfrom=0, sampto=650000,)    
annotation = wfdb.rdann(filename, 'atr', sampfrom=0, sampto=650000,shift_samps=True)
...
patient_result = result.tolist()
print(f'{len(patient_result)} peaks')

#save peak data in csv

with open(f'{patient_number}_peak.csv', 'w', newline='') as csvfile:
    csvwriter = csv.writer(csvfile)

    # Write the list of integers as a single row in the CSV file
    csvwriter.writerow(patient_result)
